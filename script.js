...
masterPlay.onclick = () => {
    if (!audioElement.src) { audioElement.src = songs.filePath; document.getElementById('masterSongName').innerText = songs.songName; }
    if (audioElement.paused) { audioElement.play(); masterPlay.classList.replace('fa-play-circle', 'fa-pause-circle'); }
    else { audioElement.pause(); masterPlay.classList.replace('fa-pause-circle', 'fa-play-circle'); }
};
...
document.getElementById('myProgressBar').onchange = () => { audioElement.currentTime = document.getElementById('myProgressBar').value * audioElement.duration / 100; };
